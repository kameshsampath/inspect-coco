...
TASK_PY_TEMPLATE = '''\
"""Eval task entry point (auto-generated by inspect-coco scaffold)."""

from inspect_ai import task

from inspect_coco.tasks import coco_task


@task
def {func_name}():
    return coco_task(task_dir=str(__file__.rsplit("/", 1)[0]))()
'''
...
def generate_suite(
    ...
    Creates:
        ...
        output_dir/<skill-name>/basic-prompt/task.py
    ...
    func_name = skill.name.replace("-", "_") + "_basic"
    task_py_content = TASK_PY_TEMPLATE.format(func_name=func_name)
    files.append(task_py)
